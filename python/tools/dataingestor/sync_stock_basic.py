#!/usr/bin/env python3
"""
Stock basic information synchronization task.
...
def main():
    """Main entry point for stock basic sync task."""
    parser = argparse.ArgumentParser(
        description="Synchronize stock basic information from Tushare to database"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        default="L",
        choices=["L", "D", "P", ""],
        help="List status: L=listed, D=delisted, P=pause, empty=all",
    )
    parser.add_argument(
        "--task-name",
        type=str,
    ...
    if not task_name:
        ...
        status_suffix = f"_{args.list_status}" if args.list_status else ""
        task_name = f"stock_basic_sync{exchange_suffix}{status_suffix}"

    logger.info("=" * 60)
    logger.info("Stock Basic Information Synchronization Task")
    logger.info("=" * 60)
    logger.info(f"Task Name: {task_name}")
    logger.info(f"Exchange: {args.exchange or 'All'}")
    logger.info(f"List Status: {args.list_status or 'All'}")
    ...
    try:
        with StockIngestorService(
            ...
            if final_state:
                ...
                logger.info(f"Last Processed Key: {final_state.last_processed_key or 'N/A'}")
            ...
            try:
                ...
                with stock_repo:
                    ...
                    if args.list_status:
                        ...
                        params["is_listed"] = (args.list_status == "L")
                    
                    where_clause = ""
                    if conditions:
                        where_clause = "WHERE " + " AND ".join(conditions)
                    
                    engine = stock_repo._get_engine()
                    table_name = stock_repo._get_full_table_name()
                    ...
                    if args.list_status:
                        logger.info(f"Filter: list_status = {args.list_status}")
                    logger.info("=" * 60)
            except Exception as e:
