...
class BinarySphericalQuantizer(nn.Module):
    ...
    def codes_to_group_indexes(self, zhat):
        """
        Convert a code to a list of indexes (in groups) in the codebook.

        Args:
            zhat: A tensor of shape (B, ..., C) containing the codes. Must be in {-1, 1}.

        Returns:
            torch.Tensor: Group index tensor.
        """
    ...
    def indexes_to_codes(self, indices):
        """
        Convert indices to codes (inverse of codes_to_indexes).

        Args:
            indices: Index tensor.

        Returns:
            torch.Tensor: Code tensor in {-1, 1}.
        """
    ...
    def group_indexes_to_codes(self, group_indices):
        """
        Convert group indices to codes (inverse of codes_to_group_indexes).

        Args:
            group_indices: Group index tensor.
...
class HierarchicalEmbedding(nn.Module):
    ...
    def split_token(self, token_ids: torch.Tensor, s2_bits: int):
        """
        Split composite token IDs into s1 and s2 tokens.

        Args:
            token_ids: Composite token IDs of shape [batch_size, seq_len] or [N],
                      each in range [0, 2^(s1_bits + s2_bits) - 1].
            s2_bits: Number of low bits used for the fine token (s2).

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: (s1_ids, s2_ids) tuple.
        """
    ...
    def forward(self, token_ids):
        """
        Forward pass of hierarchical embedding.

        Args:
            token_ids: Either:
                - tuple or list: (s1_ids, s2_ids), each of shape [batch_size, seq_len], or
                - torch.Tensor: composite token IDs of shape [batch_size, seq_len],
                  which will be split into (s1_ids, s2_ids) internally.
...
class DependencyAwareLayer(nn.Module):
    ...
    def forward(self, hidden_states, sibling_embed, key_padding_mask=None):
        """
        Forward pass of dependency-aware layer.

        Args:
            hidden_states: Hidden states tensor of shape [batch, seq_len, d_model].
