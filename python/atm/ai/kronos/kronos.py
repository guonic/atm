...
class KronosTokenizer(nn.Module, PyTorchModelHubMixin):
    ...
    def indices_to_bits(self, x, half=False):
        ...
        if half:
            ...
            # Extract bits for the first half
            x1 = (x1.unsqueeze(-1) & mask) != 0
            # Extract bits for the second half
            x2 = (x2.unsqueeze(-1) & mask) != 0
            # Concatenate the bit representations
        ...
        # Convert boolean to bipolar (-1, 1)
        x = x.float() * 2 - 1
        # Scaling factor
...
def calc_time_stamps(x_timestamp):
    """
    Calculate time stamp features from datetime.

    Args:
        x_timestamp: DatetimeIndex, Series, or array-like of datetime objects.

    Returns:
        DataFrame with time features (minute, hour, weekday, day, month).
    """
    ...
    if isinstance(x_timestamp, pd.Series):
        ...
        x_series = pd.Series(x_timestamp)
        time_df['minute'] = x_series.dt.minute
        time_df['hour'] = x_series.dt.hour
        time_df['weekday'] = x_series.dt.weekday
        time_df['day'] = x_series.dt.day
        time_df['month'] = x_series.dt.month
    return time_df
