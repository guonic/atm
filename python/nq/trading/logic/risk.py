...
class RiskManager:
    ...
    def _check_liquidity(
        ...
        if not mdf.has_symbol(order.symbol):
            self.log_event(order, "REJECT_NO_DATA")
            return False
        
        try:
            # Get symbol data (get latest available data)
            symbol_data = mdf.get_symbol_data(order.symbol)
    ...
    def _check_concentration(
        ...
        if order.side == OrderSide.BUY:
            ...
            try:
                ...
                # Try to get $close, fallback to close
                current_price = mdf.get_field(order.symbol, None, '$close')
