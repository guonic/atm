...
def _get_position_price(symbol: str, date: pd.Timestamp, market_data: pd.DataFrame) -> Optional[float]:
    """
    Get position price for a symbol on a specific date.
    
    Handles cases where the date was filtered out due to NaN values or stock suspension.
    In such cases, returns None (Position.calculate_market_value will use entry_price as fallback).
...
def run_custom_backtest(
    ...
    # This eliminates all isinstance checks downstream
    market_data = normalize_qlib_features_result(raw_market_data, instruments=instruments)
    logger.debug(f"Normalized market data: index={type(market_data.index).__name__}, columns={type(market_data.columns).__name__}")
    ...
    for i, date in enumerate(calendar):
        ...
        daily_data_for_strategy = mdf.filter_by_date(date_ts)
